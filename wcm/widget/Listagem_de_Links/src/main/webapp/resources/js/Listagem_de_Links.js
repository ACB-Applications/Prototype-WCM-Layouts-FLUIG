var Widget_Listagem_de_links = SuperWidget.extend({
  ...
  renderizarCards: function(){
	  ...
	  var url_ajax = '/api/public/ecm/dataset/search?datasetId='+nm_fichario+'&likeField=metadata_active,true';
  			...
  			if(data != null && data != undefined && data.content.length > 0){
  				var cont = 1;
					...
					for (var s = 0; s < data.content.length; s++) {
						...
						var element = '<a target="_blank" href='+endereco+' class="list-group-item"> <i class="flaticon flaticon-link icon-sm" aria-hidden="true"></i> &nbsp; '+titulo+'</a>';
  ...
  createCard: function(fields){
  	...
  	$.ajax({
          ...
          error : function(xhr, status, error) {
              FLUIGC.toast({
                  message : "Erro ao criar a ficha, favor entrar em contato com um administrador",
                  type : "danger"
              });
          }
      });
  ...
  removerFicha: function(nr_ficha){
  	...
  	$.ajax({
  		...
  		error: function(e) {
  			FLUIGC.toast({
					title: '',
					message: "Erro ao remover o link, favor entrar em contato com um administrador",
					type: 'danger'
				});
  		},
  			...
  			if(data != null && data != undefined && data.content.length > 0){
  				var cont = 1;
					...
					for (var s = 0; s < data.content.length; s++) {
						...
						var liItem = '<li class="list-group-item" id='+nr_ficha+'><i class="flaticon flaticon-link icon-sm" aria-hidden="true"></i><a href='+endereco+' target="_blank">&nbsp; '+titulo+' &nbsp;</a><button type="button" style="display: absolute; float: right; border: none; background: none;"><i class="flaticon flaticon-minus icon-sm" aria-hidden="true"></i></button></li>'
  ...
  carregarLinks: function(){
	  ...
	  for (var x=0;x<enderecos.length;x++){
		  var liItem = '<li class="list-group-item" id='+ids[x]+'><i class="flaticon flaticon-link icon-sm" aria-hidden="true"></i><a href='+enderecos[x]+' target="_blank">&nbsp; '+titulos[x]+' &nbsp;</a><button type="button" style="display: absolute; float: right; border: none; background: none;"><i class="flaticon flaticon-minus icon-sm" aria-hidden="true"></i></button></li>'
  ...
  renderizarLinks: function(){
	  ...
	  for (var x=0;x<enderecos.length;x++){
		  var element = '<a target="_blank" href='+enderecos[x]+' class="list-group-item"> <i class="flaticon flaticon-link icon-sm" aria-hidden="true"></i> &nbsp; '+titulos[x]+'</a>';
		  $("#div_links").append(element);
	  }
  ...
  addLink: function () {
	  ...
	  if(dadosCampos.EnderecoURL == "" || dadosCampos.TituloURL == ""){
		  ...
		  var liItem = '<li class="list-group-item" id='+that.contadorLink+'><i class="flaticon flaticon-link icon-sm" aria-hidden="true"></i><a href='+dadosCampos.EnderecoURL+' target="_blank">&nbsp; '+dadosCampos.TituloURL+' &nbsp;</a><button type="button" style="display: absolute; float: right; border: none; background: none;"><i class="flaticon flaticon-minus icon-sm" aria-hidden="true"></i></button></li>'
		  $("#ListaLinks_" + this.instanceId).append(liItem);
